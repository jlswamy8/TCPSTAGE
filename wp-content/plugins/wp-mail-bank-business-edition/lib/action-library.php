			...
			switch(esc_attr($_REQUEST["param"]))
			{
				...
				case "mail_bank_email_configuration_settings_module":
					if(wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "mail_bank_email_configuration_settings"))
					{
						...
						$update_email_configuration_array["reply_to"] = esc_attr($form_data["ux_txt_reply_to"]);
						$update_email_configuration_array["cc"] = esc_attr($form_data["ux_txt_cc"]);
						$update_email_configuration_array["bcc"] = esc_attr($form_data["ux_txt_bcc"]);
				...
				case "mail_bank_email_logs_date_module":
					if(wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "mb_start_end_data_email_logs"))
					{
						...
						$email_details = get_mail_bank_details_unserialize($email_manage,$mb_date1,$mb_date2);
						foreach($email_details as $value)
						{
							...
							<tr>
								...
								<td class="custom-alternative">
									...
									{
										?>
										<a href="admin.php?page=mb_email_debugging_output&log_id=<?php echo $value['id']?>">
											<i class="icon-custom-doc tooltips" data-original-title="<?php echo $mb_email_logs_show_outputs ;?>" data-placement="top"></i>
										</a> |
										<?php
									}
									?>
									<a href="admin.php?page=mb_email_log_details&log_id=<?php echo $value['id']?>">
				...
				case "mail_bank_roles_and_capabilities_module":
					if(wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "mail_bank_roles_capabilities"))
					{
						parse_str(isset($_REQUEST["data"]) ? base64_decode($_REQUEST["data"]) : "",$roles_array);
						$other_roles_array = isset($_REQUEST["roles_names"]) ? json_decode(stripslashes(html_entity_decode($_REQUEST["roles_names"]))) : "";
						$update_roles = array();
						$where = array();

						$roles = isset($roles_array["ux_chk_administrator"]) ? "1," : "1,";
						$roles .= isset($roles_array["ux_chk_author"]) ? "1," : "0,";
						$roles .= isset($roles_array["ux_chk_editor"]) ? "1," : "0,";
						$roles .= isset($roles_array["ux_chk_contributor"]) ? "1," : "0,";
						$roles .= isset($roles_array["ux_chk_subscriber"]) ? "1," : "0,";
						$roles .= isset($roles_array["ux_chk_others_privileges"]) ? "1" : "0";

						$update_roles["roles_and_capabilities"] = $roles;
						$update_roles["show_mail_bank_top_bar_menu"] = esc_attr($roles_array["ux_ddl_mail_bank_menu"]);

						$update_roles["administrator_privileges"] = "1,1,1,1,1,1,1,1,1";

						$author_role = isset($roles_array["ux_chk_full_control_author"]) ? "1," : "0,";
						if(isset($roles_array["ux_chk_full_control_author"]))
						{
							$author_role = "1,1,1,1,1,1,1,1,1";
						}
						else
						{
							$author_role .= isset($roles_array["ux_chk_email_configuration_author"]) ? "1," : "0,";
							$author_role .= isset($roles_array["ux_chk_test_email_author"]) ? "1," : "0,";
							$author_role .= isset($roles_array["ux_chk_email_logs_author"]) ? "1," : "0,";
							$author_role .= isset($roles_array["ux_chk_settings_author"]) ? "1," : "0,";
							$author_role .= isset($roles_array["ux_chk_roles_and_capabilities_author"]) ? "1," : "0,";
							$author_role .= isset($roles_array["ux_chk_system_information_author"]) ? "1," : "0,";
							$author_role .= isset($roles_array["ux_chk_error_logs_author"]) ? "1," : "0,";
							$author_role .= isset($roles_array["ux_chk_licensing_author"]) ? "1" : "0";
						}
						$update_roles["author_privileges"] = $author_role;

						$editor_role = isset($roles_array["ux_chk_full_control_editor"]) ? "1," : "0,";
						if(isset($roles_array["ux_chk_full_control_editor"]))
						{
							$editor_role = "1,1,1,1,1,1,1,1,1";
						}
						else
						{
							$editor_role .= isset($roles_array["ux_chk_email_configuration_editor"]) ? "1," : "0,";
							$editor_role .= isset($roles_array["ux_chk_test_email_editor"]) ? "1," : "0,";
							$editor_role .= isset($roles_array["ux_chk_email_logs_editor"]) ? "1," : "0,";
							$editor_role .= isset($roles_array["ux_chk_settings_editor"]) ? "1," : "0,";
							$editor_role .= isset($roles_array["ux_chk_roles_and_capabilities_editor"]) ? "1," : "0,";
							$editor_role .= isset($roles_array["ux_chk_system_information_editor"]) ? "1," : "0,";
							$editor_role .= isset($roles_array["ux_chk_error_logs_author"]) ? "1," : "0,";
							$editor_role .= isset($roles_array["ux_chk_licensing_editor"]) ? "1" : "0";
						}
						$update_roles["editor_privileges"] = $editor_role;

						$contributor_role = isset($roles_array["ux_chk_full_control_contributor"]) ? "1," : "0,";
						if(isset($roles_array["ux_chk_full_control_contributor"]))
						{
							$contributor_role = "1,1,1,1,1,1,1,1,1";
						}
						else
						{
							$contributor_role .= isset($roles_array["ux_chk_email_configuration_contributor"]) ? "1," : "0,";
							$contributor_role .= isset($roles_array["ux_chk_test_email_contributor"]) ? "1," : "0,";
							$contributor_role .= isset($roles_array["ux_chk_email_logs_contributor"]) ? "1," : "0,";
							$contributor_role .= isset($roles_array["ux_chk_settings_contributor"]) ? "1," : "0,";
							$contributor_role .= isset($roles_array["ux_chk_roles_and_capabilities_contributor"]) ? "1," : "0,";
							$contributor_role .= isset($roles_array["ux_chk_system_information_contributor"]) ? "1," : "0,";
							$contributor_role .= isset($roles_array["ux_chk_error_logs_author"]) ? "1," : "0,";
							$contributor_role .= isset($roles_array["ux_chk_licensing_contributor"]) ? "1" : "0";
						}
						$update_roles["contributor_privileges"] = $contributor_role;

						$subscriber_role = isset($roles_array["ux_chk_full_control_subscriber"]) ? "1," : "0,";
						if(isset($roles_array["ux_chk_full_control_subscriber"]))
						{
							$subscriber_role = "1,1,1,1,1,1,1,1,1";
						}
						else
						{
							$subscriber_role .= isset($roles_array["ux_chk_email_configuration_subscriber"]) ? "1," : "0,";
							$subscriber_role .= isset($roles_array["ux_chk_test_email_subscriber"]) ? "1," : "0,";
							$subscriber_role .= isset($roles_array["ux_chk_email_logs_subscriber"]) ? "1," : "0,";
							$subscriber_role .= isset($roles_array["ux_chk_settings_subscriber"]) ? "1," : "0,";
							$subscriber_role .= isset($roles_array["ux_chk_roles_and_capabilities_subscriber"]) ? "1," : "0,";
							$subscriber_role .= isset($roles_array["ux_chk_system_information_subscriber"]) ? "1," : "0,";
							$subscriber_role .= isset($roles_array["ux_chk_error_logs_subscriber"]) ? "1," : "0,";
							$subscriber_role .= isset($roles_array["ux_chk_licensing_subscriber"]) ? "1" : "0";
						}
						$update_roles["subscriber_privileges"] = $subscriber_role;

						$other_roles_capabilities = isset($roles_array["ux_chk_full_control_other_privileges_roles"]) ? "1," : "0,";
						if(isset($roles_array["ux_chk_full_control_other_privileges_roles"]))
						{
							$other_roles_capabilities = "1,1,1,1,1,1,1,1,1";
						}
						else
						{
							$other_roles_capabilities .= isset($roles_array["ux_chk_email_configuration_others"]) ? "1," : "0,";
							$other_roles_capabilities .= isset($roles_array["ux_chk_test_email_others"]) ? "1," : "0,";
							$other_roles_capabilities .= isset($roles_array["ux_chk_email_logs_others"]) ? "1," : "0,";
							$other_roles_capabilities .= isset($roles_array["ux_chk_settings_others"]) ? "1," : "0,";
							$other_roles_capabilities .= isset($roles_array["ux_chk_roles_and_capabilities_others"]) ? "1," : "0,";
							$other_roles_capabilities .= isset($roles_array["ux_chk_system_information_others"]) ? "1," : "0,";
							$other_roles_capabilities .= isset($roles_array["ux_chk_error_logs_author"]) ? "1," : "0,";
							$other_roles_capabilities .= isset($roles_array["ux_chk_licensing_others"]) ? "1" : "0";
						}
						$update_roles["other_roles_privileges"] = $other_roles_capabilities;

						$update_roles["others_full_control_capability"] = isset($roles_array["ux_chk_full_control_other_roles"]) ? "1" : "0";

						$update_roles["capabilities"] = $other_roles_array;

						$update_data = array();
						$where["meta_key"] = "roles_and_capabilities";
						$update_data["meta_value"] = serialize($update_roles);
						$obj_dbHelper_mail_bank->updateCommand(mail_bank_meta(),$update_data,$where);
					}
				break;

				case "mail_bank_email_logs_bulk_delete_module":
					if(wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "mb_email_logs_bulk_delete"))
					{
						...
						$obj_dbHelper_mail_bank->bulk_deleteCommand(mail_bank_meta(),"id",$email_logs_ids);
					}
				break;
				...
				case "mail_bank_email_logs_delete_module":
					if(wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "mb_email_logs_delete"))
					{
						...
						$obj_dbHelper_mail_bank->deleteCommand(mail_bank_meta(),$where_meta);
					}
				break;
				...
				case "mail_bank_licensing_module" :
					if(wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "mail_bank_licensing"))
					{
						parse_str(isset($_REQUEST["data"]) ? base64_decode($_REQUEST["data"]) : "",$form_data);
						$update_license = array();
						$where = array();

						$update_license["type"] = "Mail Bank Business Edition";
						$update_license["version"] = "2.1.16";
						$update_license["site_url"] = site_url();
						$update_license["api_key"] = esc_html($form_data["ux_txt_api_key"]);
						$update_license["order_id"] = intval($form_data["ux_txt_order_id"]);

						$where["licensing_id"] = isset($_REQUEST["license_id"]) ? intval($_REQUEST["license_id"]) : 0;
						$obj_dbHelper_mail_bank->updateCommand(mail_bank_licensing(),$update_license,$where);

						update_option("mail_bank_api_details",$update_license["api_key"]);
					}
				break;
			}
			die();
