...
else
{
	...
	else
	{
		...
		<script type="text/javascript">
			...
			$licensing_url = $isLicensed == "" ? "mb_licensing" : esc_attr($_GET["page"]);
			if(isset($_GET["page"]))
			{
				switch($licensing_url)
				{
					...
					case "mb_email_logs":
						...
						if(email_logs_mail_bank == 1)
						{
							...
							if(typeof(bulk_delete_email_logs) != "function")
							{
								function bulk_delete_email_logs()
								{
									var view_records_bulk_action = jQuery("#ux_ddl_email_logs").val();
									if(view_records_bulk_action == "")
									{
										var shortCutFunction = jQuery("#toastTypeGroup_error input:checked").val();
										var $toast = toastr[shortCutFunction](<?php echo json_encode($mb_choose_action); ?>);
									}
									else
									{
										var checkbox_array = [];
										jQuery("input[type=checkbox][name*=ux_chk_email_logs_]", oTable.fnGetFilteredNodes()).each(function()
										{
											if(jQuery(this).val() != "")
											{
										...
										{
											var shortCutFunction = jQuery("#toastTypeGroup_error input:checked").val();
											var $toast = toastr[shortCutFunction](<?php echo json_encode($mb_choose_record_to_delete); ?>);
										}
										else
										{
											var confirm_delete = confirm(<?php echo json_encode($mb_confirm_bulk_delete); ?>);
											if(confirm_delete == true)
											{
												overlay_loading_mail_bank(<?php echo json_encode($mb_bulk_delete_email_logs);?>);
												jQuery.post(ajaxurl,
												{
													encrypted_records: JSON.stringify(checkbox_array),
													param: "mail_bank_email_logs_bulk_delete_module",
													action: "mail_bank_action",
													_wp_nonce: "<?php echo $mb_email_logs_bulk_delete; ?>"
												},
												function(data)
												{
													setTimeout(function()
							...
							if(typeof(delete_email_logs) != "function")
							{
								...
								{
									var confirm_delete = confirm(<?php echo json_encode($mb_confirm_single_delete); ?>);
									if(confirm_delete == true)
									{
										overlay_loading_mail_bank(<?php echo json_encode($mb_delete_log);?>);
										jQuery.post(ajaxurl,
										{
											id: id,
											param: "mail_bank_email_logs_delete_module",
											action: "mail_bank_action",
											_wp_nonce: "<?php echo $mb_email_logs_delete_log; ?>"
										},
										function(data)
										{
											setTimeout(function()
							...
							({
								...
								submitHandler:function(form)
								{
									...
									jQuery.post(ajaxurl,
									{
										data: base64_encode(jQuery("#ux_frm_email_logs").serialize()),
										param: "mail_bank_email_logs_date_module",
										action: "mail_bank_action",
										_wp_nonce: "<?php echo $mb_start_end_data_email_logs ;?>"
									},
									function(data)
									{
										jQuery("#ux_chk_all_email_logs").removeAttr("checked");
										remove_overlay_mail_bank();
										jQuery("#ux_tbl_email_logs").dataTable().fnDestroy();
										jQuery("#ux_dynamic_email_logs_table_filter").html(data);
										jQuery("#ux_tbl_email_logs").dataTable();
										jQuery(".tooltips").tooltip_tip({placement: "right"});
									});
								}
							});
					...
					case "mb_error_logs" :
						...
						{
							?>
							jQuery(".btn_clear_error_logs").click(function()
							{
				  ...
				  break;
					case "mb_licensing":
						...
						<?php
						if(licensing_mail_bank == "1")
						{
							?>
							var mb_frm_licensing = jQuery("#ux_frm_licensing");
							mb_frm_licensing.validate
							({
								rules:
								{
									ux_txt_api_key:
								...
								submitHandler: function(form)
								{
									var License_id = "<?php echo intval($mb_licensing->licensing_id) ?>";
									var licenseUrl = "<?php echo tech_banker_url."/wp-admin/admin-ajax.php" ; ?>";
									jQuery.post(licenseUrl,
									{
										ux_product_key : 9292,
										data: jQuery("#ux_frm_licensing").serialize(),
										param: "license",
										action: "license_verifier"
									},
									function(data)
									{
										if(data == "")
										{
											jQuery.post(ajaxurl,
											{
												data: base64_encode(jQuery("#ux_frm_licensing").serialize()),
												license_id: License_id,
												param: "mail_bank_licensing_module",
												action: "mail_bank_action",
												_wp_nonce:"<?php echo $mb_license;?>"
											},
											function(data)
											{
												overlay_loading_mail_bank(<?php echo json_encode($mb_license_activated); ?>);
												setTimeout(function()
												{
													remove_overlay_mail_bank();
													window.location.href = "admin.php?page=mb_email_configuration";
										...
										{
											overlay_loading_mail_bank();
											jQuery("#error_notice").attr("style","");
											setTimeout(function()
											{
												remove_overlay_mail_bank();
											}, 3000);
											jQuery("#ux_error_message_from_server").html(data);
										}
									});
								}
							});
