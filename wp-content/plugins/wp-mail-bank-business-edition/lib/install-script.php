...
else
{
	...
	else
	{
		...
		*/
		...
		/*
		Function Name: licensing_table_for_mail_bank
		Parameter: No
		Description: This Function is used for creating a table in database for licensing.
		Created On: 13-08-2016 10:41
		Created By: Tech Banker Team
		*/

		if(!function_exists("licensing_table_for_mail_bank"))
		{
			function licensing_table_for_mail_bank()
			{
				$obj_dbHelper_install_script_mail_bank = new dbHelper_install_script_mail_bank();
				$sql = "CREATE TABLE IF NOT EXISTS ". mail_bank_licensing() ."
				(
				...
				$array=array();
				$array["version"] = "2.1.16";
				$array["type"] = "Mail Bank Business Edition";
				$array["site_url"] = site_url();

				$obj_dbHelper_install_script_mail_bank->insertCommand(mail_bank_licensing(),$array);
			}
		}
		...
		switch($mail_bank_version_number)
		{
			case "":
				if(count($wpdb->get_var("SHOW TABLES LIKE '". $wpdb->prefix."mail_bank'")) != 0)
				{
					...
					$update_plugin_data["remove_tables_at_uninstall"] = isset($plugin_settings_data_unserialize["remove_tables_at_uninstall"]) ? $plugin_settings_data_unserialize["remove_tables_at_uninstall"] : "disable";
			...
			else
			{
				...
				(
					"SELECT order_id FROM ".mail_bank_licensing()
				);

				$data = array();
				$where = array();
				$data["version"] = "2.1.16";
				$data["type"] = "Mail Bank Business Edition";
				$data["site_url"] = site_url();
				$where["order_id"] = $order_id;

				$obj_dbHelper_install_script_mail_bank->updateCommand(mail_bank_licensing(),$data,$where);
			}
		...
		update_option("mail-bank-version-number","2.4");
	}
}
?>
